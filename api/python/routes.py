...
def index():
    ...
    # Fetch active promos whose date range includes today
    ...
    return render_template('index.html', homepage_images=homepage_images, carousel_interval=settings.carousel_interval, promos=promos)
...
def login_options():
    """Page to choose between admin and user login"""
...
def user_login():
    ...
    if form.validate_on_submit():
        try:
            ...
            # Handle non-existent users and rejected users the same way
            if not user or (user and user.registration_status == 'rejected'):
...
def admin_login():
    ...
    form = LoginForm()
    register_form = RegistrationForm()  # Add this even though admin won't use it
    
    if form.validate_on_submit():
        try:
            user = User.query.filter_by(email=form.email.data).first()
            print(f"Login attempt - Email: {form.email.data}")  # Debug print
            ...
            if user and user.is_admin:
                ...
                print(f"Stored hash: {user.password}")  # Debug print
                ...
                flash('Email not found or not authorized as admin', 'error')
        except Exception as e:
...
def complete_registration():
    try:
        ...
        # Save ID picture
        if id_picture:
            filename = secure_filename(f"{int(time.time())}_{id_picture.filename}")
            upload_folder = os.path.join(app.config['UPLOAD_FOLDER'], 'ids')
...
def dashboard():
    ...
    try:
        ...
        return render_template('dashboard.html', bookings=bookings, room_types=room_types, food_service_ipv4=current_app.config.get('FOOD_SERVICE_IPV4'))
    except Exception as e:
        print(f"Error loading dashboard: {e}")
        import traceback
        traceback.print_exc()
        flash("Error loading your bookings. Please try again later.", "error")
        return render_template('dashboard.html', bookings=[], food_service_ipv4=current_app.config.get('FOOD_SERVICE_IPV4'))
...
def order_food():
    ...
    if resp.get('success') and resp.get('order_id'):
        php_ip = current_app.config.get('FOOD_SERVICE_IPV4', '192.168.1.6')
...
def api_cancel_food_order(order_id):
    result = cancel_food_order(order_id)
    return jsonify(result)
...
@app.route('/api/food-rating', methods=['GET'])
def get_food_rating_route():
    order_id = request.args.get('order_id')
    menu_item_id = request.args.get('menu_item_id')
    user_id = session.get('user_id')
    result = get_food_rating(user_id, order_id, menu_item_id)
    return jsonify(result)

@app.route('/api/food-rating', methods=['POST'])
def post_food_rating_route():
    data = request.json
    ...
    result = post_food_rating(user_id, order_id, menu_item_id, rating, comment)
    return jsonify(result)